...
def get_intersection_node(head1: ListNode, head2: ListNode) -> ListNode:
    ...
    method complexity: Runtime 147 ms; Memory 30.3 MB
    """
    intersection_node = None
    cur = head1
    set_val_head1 = set()
    while cur:
        set_val_head1.add(cur)
        cur = cur.next

    cur = head2
    while cur:
        if cur not in set_val_head1:
            cur = cur.next
        else:
            intersection_node = cur
            break
...
if __name__ == '__main__':
    ...
    print('actual {} expected 8'.format(get_intersection_node(head1, head2)))

    common = ListNode(2, ListNode(4))
    head1 = ListNode(1, ListNode(9, ListNode(1, common)))
    head2 = ListNode(3, common)
    print('actual {} expected 2'.format(get_intersection_node(head1, head2)))

    head1 = ListNode(2, ListNode(6, ListNode(4)))
    head2 = ListNode(1, ListNode(5))
    print('actual {} expected None'.format(get_intersection_node(head1, head2)))
