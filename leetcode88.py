...
def merge(nums1: list, nums2: list):
    ...
    method complexity: Runtime 39 ms; Memory 13.8 MB
    """
    i = k = n = 0
    temp = [0] * len(nums1)
    l = len(nums1) - len(nums2)
    while i < l and k < len(nums2):
        if nums1[i] <= nums2[k]:
            temp[n] = nums1[i]
            i += 1
            n += 1
...
if __name__ == '__main__':
    ...
    nums2 = [2, 5, 6]
    merge(nums1, nums2)
    print('actual {} expected [1, 2, 2, 3, 5, 6]'.format(nums1))

    nums1 = [1]
    nums2 = []
    merge(nums1, nums2)
    print('actual {} expected [1]'.format(nums1))
